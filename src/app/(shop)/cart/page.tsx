...
// Import global reactive cart state hook according to FSD layers architecture
import { useCartStore } from "@/features/cart/model/cartStore";

export default function CartPage() {
  // Use token state to safely track client-side hydration compliance
  const [renderToken, setRenderToken] = useState("loading");
  
  // Bind live state streams and update handlers from our Zustand store
  const { items, updateQuantity, removeFromCart } = useCartStore();

  // Defer client-side token swap to avoid synchronous cascading layout updates
  useEffect(() => {
  ...
  // Compute live subtotal dynamically across all line-items
  const totalPrice = items.reduce((acc, item) => acc + item.price * item.quantity, 0);
  
  // Luxury brand logic parameters
  ...
  if (renderToken === "loading") {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center text-zinc-400 font-sans">
        Loading premium checkout stream...
      </div>
    );
  }

  // Render empty cart fallback block state gracefully
  if (items.length === 0) {
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 flex flex-col items-center justify-center text-center font-sans animate-in fade-in duration-300">
        ...
        <p className="text-sm text-zinc-500 max-w-sm leading-relaxed mb-8">
          Discover our elegant lingerie collections to find your perfect combination of exquisite style and support.
        </p>
        <Link 
          href="/catalog" 
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans text-zinc-900 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        {/* 🛒 LEFT ZONE: LIVE PRODUCTS LIST STREAM */}
              ...
              <div className="flex flex-col flex-1 gap-1 min-w-0">
                <div className="flex justify-between items-start gap-2">
                  ...
                  <button 
                    ...
                    aria-label="Remove line item target allocation entirely"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>

                {/* Selected Attributes Badge Rows */}
                ...
                {/* Quantity Controls & Dynamic Item Price Line */}
        ...
        {/* 📊 RIGHT ZONE: EXQUISITE ORDER SUMMARY BILLING CARD */}
        <div className="lg:col-span-5 bg-zinc-50 border border-zinc-100 rounded-2xl p-6 md:p-8 sticky top-24">
          ...
          <div className="flex flex-col gap-4 text-sm mb-6">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold text-zinc-900">{totalPrice} UAH</span>
            </div>
            ...
            <div className="flex flex-col gap-1.5 pb-4 border-b border-zinc-200">
              ...
              {shippingCost > 0 && (
                <p className="text-[11px] text-zinc-400 font-light leading-snug">
                  Spend <strong className="font-medium text-[#C8205C]">{2000 - totalPrice} UAH</strong> more to unlock premium FREE shipping destination rewards.
                </p>
              )}
            </div>

            <div className="flex justify-between items-baseline pt-2">
              <span className="text-base font-bold uppercase text-zinc-900">Total</span>
              <span className="text-xl font-black text-[#C8205C]">{grandTotal} UAH</span>
            </div>
          </div>

          {/* Secure Routing Call to Action Trigger */}
          <Link
            href="/checkout"
